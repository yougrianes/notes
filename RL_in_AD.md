...
- **批次大小**：每个 GPU 16 个批次。  
- **训练轮次**：20 个训练 epoch。  
- **优化器**：使用 AdamW 优化器，起始学习率为 2e-4，每 1k 步衰减 0.95。  

## 二、奖励模型训练  
- **数据**：收集 5k 个场景，每个场景包含 50 个成对比较。  
- **批次大小**：每个 GPU 32 个批量。  
- **训练轮次**：50 个时期。  
...
- **正则化参数**：α = 10  
- **剪辑参数**：ϵ = 0.01  
- **样本大小**：每个 GPU 样本大小 m = 32  
- **学习率**：γ = 1e−5  
- **微调步骤**：T = 1000  
- **更新迭代**：每个步骤更新迭代 I = 5  

## 四、训练环境  
所有训练作业均在 8 个 NVIDIA A100 GPU 上完成。
...
1. 多种异构场景的处理效率低下
2. 预测的不确定性以及对多种可能性之间的权衡不好

无人驾驶任务里，数据以一帧一帧的流给到model，包含矢量化的高精地图和周围agent的历史轨迹。最近的factorized attention方法（时空分开分别进行attentin）将这些信息的处理提升到新高度。但这需要对于每个场景元素做attention，如果场景非常复杂，cost还是很大的。

query-centric的办法：场景元素在它们自己的时空坐标系内进行特征提取，和全局坐标系无关（ego在哪无关了）。

### 输入和输出形式

考虑自动驾驶场景中，周围有 A 个智能体。

感知模块按固定间隔向预测模块输入智能体状态，包含空间位置、角位置、时间位置、速度及运动向量等。

同时，预测模块可获取高清地图上 M 个多边形信息，包括采样点和语义属性。

输出：模型需根据 T 时间步的观察窗口内的信息，为每个目标智能体预测 K 条未来 T' 时间步的轨迹，并为每条轨迹分配概率得分。

### 以查询为中心的场景上下文编码

挑战与创新点：现有因式分解注意力用于场景编码虽有效，但计算复杂度高，且基于智能体中心的编码范式无法复用计算。Q

CNet 建立局部时空坐标系，使编码独立于全局坐标，解决上述问题。

具体步骤：

为各场景元素建立由参考时空位置和方向确定的局部坐标系；

将几何属性转换为极坐标，再变换为傅里叶特征，结合语义属性经 MLP 得到嵌入；

计算场景元素对的相对时空位置嵌入；

对地图编码采用自注意力，对智能体编码采用因式分解注意力，且所有编码具有旋转和平移不变性，可缓存和复用计算结果，降低在线推理复杂度。

### 基于查询的轨迹解码

整体架构：采用递归无锚提案模块生成自适应轨迹锚，再由基于锚的模块对初始提案进行优化。

关键组件：

提案和优化模块都使用类似 DETR 的架构，通过 Mode2Scene 和 Mode2Mode 注意力机制更新查询。

无锚轨迹提案模块使用可学习的无锚查询，以数据驱动方式生成 K 个自适应锚，且采用递归方式解码未来路标点，减轻查询负担。

基于锚的轨迹优化模块以提案模块输出为锚，预测轨迹偏移和假设可能性，为模型提供明确空间先验。
